...
v
    ...
    \chapter{Forma Polar y Argumentos}
        ...
        \section{Ley de Moivre's}
            ...
            $z ^ n = r ^ n (\cos (n\theta) + i\sin (n\theta)), \quad \text{donde } n\in \mathbb Z$
            
            % ======== DEMOSTRACION ========
            \begin{SmallIndentation}[1em]
                \textbf{Demostración}:

                Se puede dar una demostracion muy sencilla, no se porque los libros usan induccion matematica para demostrar el teorema de Moivre... en fin, expresando a $z$ en su forma polar y usando la formula de Euler, tenemos: 

                \begin{align}  %  Si se le pone * no se enumeran las ecuaciones
				  z ^ n &= (r(\cos (\theta) + i \sin (\theta)) ^ n = r ^ n(\cos (\theta) + i \sin (\theta)) ^ n   \\
				  		&= r ^ n e ^ {n\theta i} = r ^ n(\cos (n\theta) + i \sin (n\theta)) 	\\ 
				\end{align}
            \end{SmallIndentation}
    ...
    \chapter{Forma Exponencial}
        ...
        \section{Fórmula de Euler}  
            ...
            \begin{SmallIndentation}[1em]
                ...
                Pasa algo muy interesante al hacer $k=i\theta$, pues vemos que se hayan las series de McLaurin del seno y coseno:
                \begin{MultiLineEquation*}{2}
                    e^{i\theta} &= 
	...
	\chapter{Funciones complejas}
        ...
        Cualquier funcion compleja $w = f(z)$ puede ser representada como $f(z) = u(x, y) + iv(x, y)$ donde $x, y \in \Reals $
